...
class QueryClassifier:
    ...
    def classify(self, query):
        ...
        response = query_grokcloud(prompt)
        classification = "Debatable"  # Default to debate for safety
        reasoning = response
        if "Error:" in response:
            reasoning = f"Classification failed: {response}. Defaulting to Debatable."
        else:
            # Extract classification (last word for simplicity)
            last_word = response.strip().split()[-1]
            if last_word in ["Debatable", "Non-debatable"]:
...
class Orchestrator:
    ...
    def run_debate(self, query, debate_rounds=None):
        ...
        if classification_output["classification"] == "Non-debatable":
            # Non-debatable: Provide straightforward answer
            prompt = f"Provide a clear and concise answer to: {query}"
            answer = query_grokcloud(prompt)
            reasoning_log.append({
